...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const safeCosTheta = Math.min(1, Math.max(-1, cosTheta));
...
function getSumOfDigits(num) {
  ...
  // Iterate over each character and sum its integer value
...
function getRandomInteger(min, max) {
  ...
  const randomInteger = Math.floor(randomFloat * range) + min;
